...
import com.che.test.rxjava.asyncwork.Func;
...
public class RxJavaStudy {
    ...
    public AsyncWork<Uri> getUriOperator(String tag) {
        ...
        AsyncWork<Uri> uriWork = newsWork.flatMap(new Func<News, AsyncWork<Uri>>() {
            @Override
    ...
    public Observable<Uri> getUriRxLambda(String tag) {
        ...
        Observable<Uri> uriObservable = apiRx
                ...
                .map(newsList -> getLatestNews(newsList))//第2步,得到Observable<News>
                .flatMap(news -> apiRx.save(news));//第3步,得到Observable<Uri>
        return uriObservable;
    }
